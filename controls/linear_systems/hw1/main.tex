...
\begin{letterenum}
    ...
    \begin{equation}
        \boxed{\frac{d (\delta x)}{dt} \approx B \delta u}
    \end{equation}

    \item Fix $\omega(t) = v(t) = 1$, $p_x(t) = \sin(t)$,
    $p_y(t) = 1 - \cos(t)$, and $\theta(t) = t$.
    To show this is a valid trajectory for our system, we take the 
    derivatives of each curve
    \begin{align}
    ...
    \begin{equation}
        \boxed{\frac{d(\delta x(t))}{dt} = \begin{bmatrix}
            0 & 0 & -\sin(t) \\
            0 & 0 & \cos(t) \\
            0 & 0 & 0
...
\begin{letterenum}
    \item Let $\alpha, \beta$ be elements of the set $\R^{m \times n}$ with
        $m, n \in \N$. Let $a_{ij}$ and $b_{ij}$ denote the $ij$th entry of 
        the matrices $\alpha$ and $\beta$, respectively. Let $r,s \in \R$ be
        scalars. 
        ...
        We prove that the set $V = (\R_{m\times n}, +, \cdot)$ over the field
        ...
        Thus, the set $\R^{m \times n}$ with standard matrix operations $+$
        and $\cdot$ over the field $\R$ satisfies all the properties of a 
        vector space. Clearly, $\R^{2 \times 2}$ is a special case of
        $\R^{m \times n}$ and thus is also a vector space.

    \item The dimension of $\R^{m \times n}$ is $m \cdot n$ because that is
        the number of vectors in the basis for $\R^{m \times n}$, as we shall
        see in the subsequent subproblem. So the dimension of 
        $\R^{2 \times 2}$ is 4.
    ...
    \item Define $\epsilon_{IJ}$ as the matrix with the $ij$th entry containing:
        ...
        of $\mathcal{B}$
        \begin{equation}
            \beta_{ij} := \biggl(\sum_{I=1}^{m} \sum_{J=1}^{n} c_{IJ} \epsilon_{IJ}
                \biggr)_{ij} = c_{IJ}
        \end{equation}
...
0, and has non-trivial coefficients. Thus, this set of vectors is
$\boxed{$linearly dependent.$}$ 

We verify this holds by computing the linear combination of interest. Since
\begin{equation}
    A^2 = \begin{bmatrix}
        1 & 1 \\ 0 & 2
    \end{bmatrix} \begin{bmatrix}
...
We see that the Cayley-Hamilton theorem has correctly predicted a linear
combination which annihilates $A$.
...
\begin{letterenum}
    \item We rearrange the given equation for $(T(\theta))_i$ so that
        ...
        \begin{equation}
            ...
            \end{bmatrix}
        \end{equation}
        shows that $\boxed{\mathcal{B}_N = \{[1, 1, 1, 1]^T\}}$
        ...
        \item 
            ...
            We would like to express the operator
            ...
            \end{equation}
            (see section (\ref{appendix}) for notational conventions)
            ...
            \begin{align}
                ...
                \end{bmatrix}.
            \end{align}
            Since $[\alpha]_{\basis_1} = \alpha$, we will drop the bracket
            notation for vectors expressed in $\basis_1$.
            ...
            \begin{equation}
                \boxed{[T]_{\basis_2}^{\basis_2} = \begin{bmatrix}
                    -2 &  1 &  0 &  0  \\ 
                    -0.5 & -0.5 &  0.5 &  0  \\
                    -1 &  1 & -1 &  0.5  \\
                    -1 &  0 &  1 & -0.5  \\
                \end{bmatrix}}
            \end{equation}
            We discuss a more intuitive representation in Appendix section 
            (\ref{sec:intuitive-basis}).
...
Let $A\in\R^{n \times n}$, $n\in\N$. We have
\begin{equation}
    (A^i)^T = (A^T)^i
...
We now complete the proof for the main premise:
\begin{equation}
...
\begin{equation}
    AA^T = A(-A) = (-A)A = A^TA
\end{equation}
Therefore, $e^{At}$ is orthogonal if $A$ is skew-symmetric.
...
\begin{equation}
    \conv = -A^{-1}e^{-A\tau} \eval_0^t = A^{-1} (I - e^{-At})
\end{equation}
...
\end{align*}
This abuse of notation obscures the name of the argument,
e.g. in some places $c= \cos(\tau)$, but this should be
clear from context which argument is used.
...
\begin{equation}
    ...
    \end{bmatrix},
\end{equation}
a simple matrix inversion and problem 3.1(c) hold that
\begin{equation}
    A^{-1} = \begin{bmatrix}
        0 & -1 \\ 1 & 0
...
\begin{letterenum}
    ...
    and
    \begin{equation}
        \boxed{x(1) = \mat{\cos(1) + \sin(1) \\ \cos(1) - \sin(1) - 1} \approx \mat{1.382 \\ -1.301}}
    \end{equation}
    ...
    \begin{align}
        \convu &= \int_{0}^{t} \tau e^{-A\tau}d\tau \\
            ...
            &= A^{-1} (tI + A^{-1}) (I - e^{-At})
    \end{align}
    % Latex to breaks if you use \mat{}{} with two args????
    so that
    \begin{align}
        x(t) &= e^{At}x(0) + A^{-1}(tI + A^{-1})(e^{At} - I)B \\
            &= \mat{c & s \\ -s & c}\mat{1 \\ 0} +
                \mat{0 & -1 \\ 1 & 0}\mat{t & -1 \\ 1 & t}\mat{c - 1 \\ -s} \\
            &= \mat{c \\ -s} + \mat{-ts + (c - 1) \\ t(c-1) + s}
    \end{align}
    which evaluates to 
    \begin{equation}
        \boxed{x(1) \approx \begin{bmatrix}
            -1.841 \\ -0.460
        \end{bmatrix}}
    \end{equation}

    \item We utilize the double angle identities
    \begin{equation}
        \cos(t)\sin(t) = \frac{1}{2}\sin(2t) \qquad
            \sin^2(t) = \frac{1}{2}(1 - \cos(t2))
    \end{equation}
    to analyze the integral for $u(t) = \sin(t)$:
    \begin{align}
        \int_{0}^{t} e^{-A\tau} \sin(\tau) d\tau
            &= \int_{0}^{t} \mat{c & -s \\ s & c} s d\tau \\
            &= \int_{0}^{t} \mat{cs & -s^2 \\ s^2 & cs} d\tau \\
            &= \frac{1}{2} \int_{0}^{t} \mat{
                s_{2\tau} & c_{2\tau} - 1 \\
                1 - c_{2\tau} & s_{2\tau}
            } d\tau \\ &= \frac{1}{4} \mat{ -c_{2\tau} & s_{2\tau} - \tau \\
                \tau - s_{2\tau} & -c_{2\tau}
            } \eval_0^t \\
            &= \frac{1}{4} \biggl(\mat{
                -c_{2t} & s_{2t} - t \\
    ...
    So then,
    \begin{align}
        x(t) &= e^{At}x(0) + e^{At} \int_{0}^{t} e^{-A\tau}\sin(\tau)d\tau B \\
            &= \mat{c & s \\ -s & c} \biggr(\mat{1 \\ 0} + \frac{1}{4}\mat{
                1 - c_{2t} \\
                2t - s_{2t}
            }\biggl)
    \end{align}
    and evaluating at $t=1$ has
    \begin{equation}
        \boxed{x(1) \approx \mat{
            0.961 \\
            -0.992
        }}
    \end{equation}
...
Let $\basis_1 = \{\alpha_1, \alpha_2, \dots, \alpha_n\}$ and $\basis_2$
be two bases on a finite dimensional vector space $V$ with dimension $n$.

Let $\alpha \in V$. Then $[\alpha]_{\basis_1} \in \R^n$ shall
denote the expression of $\alpha$ as "coordinates" in $\basis_1$.

We define the change of basis formula using a notation
inspired by \cite{bullo_lectures_2022}.


{\it Definition}: $[P]_{\basis_2}^{\basis_1}$ to denote the expression
of $\basis_1$ with respect to $\basis_2$. In other words, the $j$th column
of $[P]_{\basis_2}^{\basis_1}$ is $[\alpha_j]_{\basis_2}$.
...
{\it Theorem:}
We transform between coordinates in different bases with via the relation:
\begin{equation}
    [\alpha]_{\basis_2} = [P]_{\basis_2}^{\basis_1} [\alpha]_{\basis_1}
\end{equation}

A proof is provided in \cite{hoffman_linear_2014} Ch 2, Thm 7.
...
\subsection{Intuitive Basis Selection}
\label{sec:intuitive-basis}
For the author's own clarification, we continue problem (2.3) by noting that
basis $\basis_2$ differs from
...
\begin{equation}
    \basis_3 = \begin{Bmatrix}
    \epsilon_1, &\qquad
    \epsilon_1 + \epsilon_2, &\qquad
    \epsilon_1 + \epsilon_2 + \epsilon_3, &\qquad
...
\begin{align}
    ...
    \end{bmatrix}
\end{align}
To understand the intuitive properties, let $\theta \in \R^4$
be the angle of each bird and
...
\begin{equation}
    ...
    \biggr.
\end{equation}
represents the difference between the heading of a pair of subsequent
bird's headings for all but the last. Given the particular
collection of neighbors in question, $\{\mathcal{N}_i: i=1,\dots,4\}$,
contains strictly adjacent birds,
