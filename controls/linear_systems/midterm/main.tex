...
\item Since we assume that $\tau(t) = 0$ for all $t \geq 0$, we also have
    ...
    \begin{equation*}
        \frac{d}{dt}{(\delta x^{**})} = \frac{\delta f}{\delta x} \delta x^{**}
            + \mathcal{O}((\delta x^{**})^2).
    \end{equation*}
...
\begin{align*}
    x(t) &= e^{At} \int_{0}^{t} e^{-A\tau} B e^{-\tau} d\tau \\
        &= P e^{Dt} P^{-1} \int_{0}^{t} P e^{-Dt} (P^{-1} B) e^{-\tau} d\tau \\
...
\begin{equation}
    \frac{d}{dt} \biggl(
        \biggl(
            \int_0^t A(\tau) d\tau
        \biggr)^i
    \biggr) = i A(t) \biggl(\int_{0}^{t} A(\tau d\tau)^{i-1}\biggr)
...
\begin{proof}
    ...
    \begin{align*}
        ...
        &= (i + 1) A(t) f_A(t)^{i}
    \end{align*}
    where we have used the product rule, the fact that $\frac{df}{dt}(t) = A(t)$,
    and the fact that $A(t)$ and $f_A(t)$ commute.
    Thus, (\ref{eq:derivative-of-ltv-integral})
    holds for all $i$, completing the proof by induction.
\end{proof}
...
multiplication and using the fact that the real number $\omega(t)$ and
$\omega(\tau)$ commute:
\begin{align*}
    A(t) A(\tau) &= \mat{
        0 & \omega(t) \\
