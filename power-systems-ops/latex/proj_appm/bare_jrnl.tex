...
\section{Case Study: IEEE 39-bus System}
...
library (JuMP). Values for linear cost coefficients (capital and operational) are assessed from EIA data\ref{} and we breakdown
capital costs of BESS technology on per energy unit basis by using the same ratio from \ref{} while updating the magnitude
...
The results show that transmission is a major limiting factor, since this is fixed parameter. The dual variables associated with these
constraints indicate the price of transmission congestion.

The resulting capacity figures $\P_\X$ and $\E_\X$ are loaded into encoord's commericial power flow
software (SAint) to validate. We find 99.9\% of load is served, which indicates that we fail to account for a few losses, but
